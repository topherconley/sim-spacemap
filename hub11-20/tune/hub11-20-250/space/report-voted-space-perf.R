...
xhub_voted_space_perf <- foreach(bhf = best_holdout_files, .packages = "spacemap") %do% { 
  bhf <- sub(pattern = "/home/cconley/", replacement = "~/", x = bhf)
  fit <- cvVoteRDS(mod_lambda = bhf, tol = 1e-6, major_thresh = 0.5, method = "space")
...
xhub_voted <- rbindlist(xhub_voted_space_perf)
...
saveRDS(xhub_voted, file = file.path(basedir, "xhub_space_top_cv_voting_performance.rds"))
save.image(file = file.path(basedir, "xhub_space_top_cv_voting.rda"))
...
simple_xhub_space_perf <- foreach(bhf = best_holdout_files, .packages = "spacemap") %do% { 
  bhf <- sub(pattern = "/home/cconley/", replacement = "~/", x = bhf)
  fit <- cvVoteRDS(mod_lambda = bhf, tol = 1e-6, major_thresh = 0.5, method = "space")
...
library(data.table)
simple_xhub_voted <- rbindlist(simple_xhub_space_perf)
simple_xhub_voted[,list(mean_mcc = mean(mcc), mean_power = mean(power), mean_fdr = mean(fdr))]
basedir <- sub("/home/cconley/", "~/", basedir)
saveRDS(simple_xhub_voted, file = file.path(basedir, "simple_xhub_space_top_cv_voting_performance.rds"))
save.image(file = file.path(basedir, "simple_xhub_space_top_cv_voting.rda"))
