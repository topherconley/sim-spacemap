...
out.seq <- pmax(1,rpois(n = R, lambda =2.2))
...
tophubs_index <- sort(hub_pag$vector, index.return = TRUE, decreasing = TRUE)$ix[1:20]
...
count_components(cgpag)

#generate weights among X--Y and Y--Y edges 
pc <- runif(n =  ecount(cgpag), min = 0.5, max =  1)
#pc <- ifelse(test = rbinom(n = ecount(cgpag), size = 1, prob = 0.5), pc, -1*pc)
hist(pc)
cgpag <- set_edge_attr(graph = cgpag, name = "weight", value = pc)
...
for (xhub in xhubs_index) { 
  ...
  tmpg <- set_vertex_attr(graph = tmpg, name = "name", value = id)
...
cgpag <- set_vertex_attr(cgpag, name = "type", index = setdiff(1:R, c(xhubs_index, yhubs_index)), value = "y")

V(cgpag)[ type %in% "xhub"]
V(cgpag)[ type %in% "yhub"]
V(cgpag)[ type %in% "x"]
V(cgpag)[ type %in% "y"]
write.graph(graph = cgpag, file = "~/tmp/cg-power-law.gml", format = "gml")
...
identical(abs(parcor_xxyy) > tol, abs(Precision) > tol)
