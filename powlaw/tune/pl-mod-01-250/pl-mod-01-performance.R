setwd("~/scratch-data/sim-spacemap/powlaw/pl-mod-01//results/n250/")
...
ggsave(filename = "~/Dropbox/Chris_Conley/ms-spacemap/figures/pl-mod-01-performance-summary.png", 
       plot = fig0)

scv <- lvoted[[3]]
scv_xxyy <- scv["(yy,xy)",]
hist(scv_xxyy$mcc)
summary(scv_xxyy$mcc)
...
ordered_levels <- c("sCGGM.CV.vote", "sCGGM.All","SPACE.All", "SPACE.CV.vote", "Spacemap.CV.vote")
spv$method <- factor(x = "SPACE.CV.vote", levels = ordered_levels)
scv$method <- factor(x = "sCGGM.CV.vote", levels = ordered_levels)
mv$method <- factor(x = "Spacemap.CV.vote", levels = ordered_levels)
lall_voted <- lapply(lall_voted, as.data.frame)
